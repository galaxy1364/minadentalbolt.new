...
export function MinadentLogo({ size = 36, className = '' }: { size?: number; className?: string }) {
  return (
    <div
      className={`inline-flex items-center justify-center shrink-0 rounded-[24%] bg-gradient-to-br from-violet-400 via-purple-600 to-indigo-700 shadow-md shadow-purple-500/25 border border-white/30 overflow-hidden ${className}`}
      style={{ width: size, height: size }}
      title="مینادنتال"
    >
      <svg width={Math.round(size * 0.72)} height={Math.round(size * 0.72)} viewBox="0 0 48 48" fill="none">
        <defs>
          <filter id="toothGlow" x="-20%" y="-20%" width="140%" height="150%">
            <feDropShadow dx="0" dy="1.5" stdDeviation="1.5" floodColor="#3b0764" floodOpacity="0.35"/>
          </filter>
        </defs>
        <g filter="url(#toothGlow)">
          <path
            ...
            fill="#ffffff"
          />
          <path d="M24 13.5 C24 20 23.6 27 23.3 32" stroke="#f3e8ff" strokeWidth="1.2" strokeLinecap="round" opacity="0.75"/>
        </g>
        <ellipse cx="20" cy="15.5" rx="3.1" ry="5.4" fill="#ffffff" opacity="0.6" transform="rotate(-18 20 15.5)"/>
      </svg>
    </div>
  )
}
