// MinadentLogo.tsx — uses the real branded tooth artwork the clinic provided
// (public/logo-256.png and public/logo-256-crisp.png).
// The tooth itself is the authentic glossy 3D dental artwork.
// The outer surround ("دورش") is framed with an elegant modern gradient
// blending teal (کله‌غازی) and soft purple/violet (بنفش ملایم) as requested.
...
export function MinadentLogo({ size = 36, className = '' }: { size?: number; className?: string }) {
  ...
  return (
    <div
      className={`relative inline-flex items-center justify-center p-[2px] rounded-[24%] bg-gradient-to-br from-teal-400 via-purple-500 to-violet-700 shadow-md shadow-purple-500/20 shrink-0 select-none ${className}`}
    ...
    >
      <img
        ...
        height={size - 4}
        alt="مینادنتال"
        className="w-full h-full object-cover rounded-[20%] block pointer-events-none"
      />
    </div>
  )
}
