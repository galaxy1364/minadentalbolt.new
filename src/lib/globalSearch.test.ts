...
import { scoreRecord, rankResults, parseQuery, groupByKind, flattenGroups, KIND_LABELS } from './globalSearch'
...
describe('flattenGroups', () => {
  it('returns the painted order, not the raw score order', () => {
    // Deliberately interleaved: a treatment outranks the second patient,
    // so the raw ranking is patient / treatment / patient while the
    // painted list is patient / patient / treatment.
    const results = rankResults('احمدی', [
      rec({ kind: 'patient', id: 'p1', title: 'احمدی' }),
      rec({ kind: 'treatment', id: 't1', title: 'احمدی', route: '/t' }),
      rec({ kind: 'patient', id: 'p2', title: 'احمد رستمی' }),
    ])
    const painted = flattenGroups(groupByKind(results))
    expect(painted.map((r) => r.kind)).toEqual(['patient', 'patient', 'treatment'])
    expect(results.map((r) => r.kind)).not.toEqual(painted.map((r) => r.kind))
  })

  it('keeps every result — nothing is dropped by grouping', () => {
    const results = rankResults('احمدی', [
      rec({ kind: 'patient', id: 'p1', title: 'احمدی' }),
      rec({ kind: 'treatment', id: 't1', title: 'احمدی', route: '/t' }),
      rec({ kind: 'labOrder', id: 'l1', title: 'احمدی', route: '/l' }),
    ])
    expect(flattenGroups(groupByKind(results))).toHaveLength(results.length)
  })
