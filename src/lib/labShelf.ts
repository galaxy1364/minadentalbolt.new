...
/** The complete patch for marking a case delivered.
 *
 * `work_done` is forced true, and that is not a convenience. The DB holds
 * `check (delivered = false or work_done = true)`, while readyForDelivery
 * also admits cases whose status is 'ready'/'received' but whose
 * work_done flag was never set — every row that predates MOD-FEAT-009
 * defaulted to false. Sending delivered alone for one of those is
 * rejected by Postgres.
 *
 * Offline-first makes that worse than a plain error: the Dexie write
 * succeeds, the operator is told it worked, and the failure only appears
 * later in the sync queue. So the invariant lives here, in tested code,
 * rather than in a JSX click handler.
 */
export function deliveryPatch(): { delivered: true; work_done: true; status: string } {
  return { delivered: true, work_done: true, status: 'delivered' }
}
