...
describe('deliveryPatch', () => {
  it('always sets work_done, because the DB constraint requires it', () => {
    // Postgres holds check (delivered = false or work_done = true).
    // Sending delivered on its own is rejected.
    const patch = deliveryPatch()
    expect(patch.delivered).toBe(true)
    expect(patch.work_done).toBe(true)
  })

  it('covers the rows that readyForDelivery admits without work_done', () => {
    // This is the case the original code got wrong: an order created
    // before MOD-FEAT-009 has work_done false by default, but a status
    // of 'ready' still puts it on the worklist. Delivering it with only
    // { delivered: true } violates the constraint — and offline-first
    // means the operator would be told it saved.
    const legacy = { id: 'l1', status: 'ready', work_done: false, delivered: false }
    expect(readyForDelivery([legacy])).toHaveLength(1)

    // Typed loosely on purpose: with the literal types from deliveryPatch
    // the compiler folds this to `true` and the assertion stops testing
    // anything at runtime.
    const after: { delivered: boolean; work_done: boolean } = { ...legacy, ...deliveryPatch() }
    expect(after.delivered === false || after.work_done === true).toBe(true)
  })
})
