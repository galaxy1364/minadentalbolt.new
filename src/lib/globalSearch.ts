...
/**
 * Flattens grouped results back into the exact order they are painted in.
 *
 * Why this exists: the list is rendered grouped, but arrow-key navigation
 * walked the raw score-sorted array. Those two orders differ whenever a
 * lower-ranked result shares a kind with a higher-ranked one, so pressing
 * ↓ made the highlight jump between sections instead of moving down one
 * row. Both the renderer and the keyboard handler read this instead.
 */
export function flattenGroups(
  groups: { kind: GlobalResultKind; items: GlobalResult[] }[],
): GlobalResult[] {
